...
export default function PlayerControls() {
  ...
  const hours = Math.floor(timeline / 3600)
    .toString()
    .padStart(2, "0");
  const minutes = Math.floor(timeline / 60)
    .toString()
    .padStart(2, "0");
  const seconds = Math.floor(timeline % 60)
    .toString()
    .padStart(2, "0");
  return (
    <div className={`${styles.controlBar} ${!areControlsVisible ? styles.hidden : ""}`}>
      <div className={styles.topPart}>
        <div className={styles.timeline}>
          {timeline > 3600 ? `${hours}:${minutes}:${seconds}` : `${minutes}:${seconds}`}
        </div>
        <ProgressBar
          max={duration}
          value={timeline}
      ...
      <div className={styles.bottomPart}>
        <div className={styles.playAndVolume}>
          <Play
            isPlaying={isPlaying}
            videoPlayHandler={onPlayToggle}
            videoPauseHandler={onPlayToggle}
            size={32}
          />
          <Volume
            onSeek={onVolumeSeek}
