  ...
  persist(
    (set) => ({
      ...
      setPlaybackRate: (rate) =>
        set(() => {
          const valid = (PLAYBACK_RATES as readonly number[]).includes(rate) ? (rate as PlaybackRate) : 1.0;
          return { playbackRate: valid };
        }),
      stepPlaybackRate: (direction) =>
        set((state) => {
          const currentIndex = PLAYBACK_RATES.indexOf(state.playbackRate);
          const nextIndex = Math.min(Math.max(currentIndex + direction, 0), PLAYBACK_RATES.length - 1);
