...
export default function Title({ name, posterUrl = "/cat.webp", type, transcodingStatus, onClick }: TitleProps) {
  const posterSrc = getOptimizedImageSrc(posterUrl);
  if (transcodingStatus !== TranscodingStatus.COMPLETED) return;
  return (
    <button type="button" onClick={() => onClick()} className={styles.title}>
      <div className={styles.posterConteiner}>
        <Image className={styles.poster} src={posterSrc} alt={name} width={150} height={225} />
