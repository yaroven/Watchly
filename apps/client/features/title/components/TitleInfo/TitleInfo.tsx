...
export default function TitleInfo({ title }: TitleInfoProps) {
  ...
  const releaseYear = createdAt && !Number.isNaN(new Date(createdAt).getTime()) ? new Date(createdAt).getFullYear() : "New";
  ...
  return (
    <div className={styles.titleInfoContainer}>
      <div className={styles.posterContainer}>
        <Image className={styles.poster} src={posterSrc} alt={name} width={320} height={460} priority />
      </div>
      <div className={styles.titleInfo}>
        <div className={styles.header}>
          ...
          <div className={styles.titleBlock}>
            <h2>{name}</h2>
            <p>Clean presentation, elevated contrast, and quick context around every title before playback starts.</p>
          </div>
        </div>
